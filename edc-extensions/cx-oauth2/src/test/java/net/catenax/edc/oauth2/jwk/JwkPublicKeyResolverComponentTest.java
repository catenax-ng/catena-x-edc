...
class JwkPublicKeyResolverComponentTest {
  ...
  private List<PublicKeyReader> jsonWebKeyReaders;
  ...
  void setUp() {
    ...
    final PublicKeyReader jsonWebKeyReader = new RsaPublicKeyReader(monitor);
    this.jsonWebKeyReaders = Collections.singletonList(jsonWebKeyReader);
    ...
    this.jwkPublicKeyResolver =
        new JwkPublicKeyResolver(
            JWKS_URI, httpClient, typeManager, monitor, jsonWebKeyReaders, INTERVAL);
  }

  @Test
  void testPublicKeyNullForKeyIdNotFound() {
    this.jwkPublicKeyResolver.start();
    final PublicKey key = this.jwkPublicKeyResolver.resolveKey("foo");
  ...
  void testPublicKeyFoundById() {
    this.jwkPublicKeyResolver.start();
    final PublicKey key = this.jwkPublicKeyResolver.resolveKey(JWKS_KEY_ID);
