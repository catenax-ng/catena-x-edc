...
class JwkPublicKeyResolverTest {
  ...
  private static final String JWKS = String.format(
      "{"
          + "   \"keys\": ["
          + "       {"
          + "           \"kty\": \"RSA\","
  ...
  void setUp() {
    ...
    Mockito.when(publicKeyReader.read(any(JsonWebKey.class))).thenAnswer((i) -> {

      final JsonWebKey jsonWebKey = i.getArgument(0);

      if (jsonWebKey.getKid().equals(JWKS_KEY_ID)) {
        final PublicKeyHolder publicKeyHolder = PublicKeyHolder.builder()
            .keyId(JWKS_KEY_ID)
        ...
        .thenAnswer(
            (i) -> {
              ...
              if (request.url().toString().equals(JWKS_URI.toString())) {
                final ResponseBody responseBody = ResponseBody.create(JWKS, MediaType.get("application/json"));
    ...
    jwkPublicKeyResolver = new JwkPublicKeyResolver(
        JWKS_URI, httpClient, typeManager, monitor, Collections.singletonList(publicKeyReader), INTERVAL);
  }
