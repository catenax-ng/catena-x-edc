...
public class HashicorpVaultExtension implements VaultExtension {
    ...
    @EdcSetting
    public static final String VAULT_HEALTH_CHECK = "edc.vault.hashicorp.health.check";
    ...
    private void configureHealthCheck(HashicorpVaultClient client, ServiceExtensionContext context) {
        final boolean healthCheckEnabled = context.getSetting(VAULT_HEALTH_CHECK, VAULT_HEALTH_CHECK_DEFAULT);
        if (!healthCheckEnabled) return;

        final HashiCorpVaultHealthCheck healthCheck = new HashiCorpVaultHealthCheck(client, context.getMonitor());
