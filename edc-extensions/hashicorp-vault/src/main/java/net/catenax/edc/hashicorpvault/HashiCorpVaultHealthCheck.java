        ...
        try {
            ...
            switch (response.getCodeAsEnum()) {
                ...
                case UNSEALED_AND_STANDBY:
                    final String standbyMsg = String.format(HEALTH_CHECK_ERROR_TEMPLATE, "Vault is in standby", response.getPayload());
                ...
                case DISASTER_RECOVERY_MODE_REPLICATION_SECONDARY_AND_ACTIVE:
                    final String recoveryModeMsg = String.format(HEALTH_CHECK_ERROR_TEMPLATE, "Vault is in recovery mode", response.getPayload());
                    monitor.debug(recoveryModeMsg);
                    return HealthCheckResult.failed(recoveryModeMsg);
                case PERFORMANCE_STANDBY:
                    final String performanceStandbyMsg = String.format(HEALTH_CHECK_ERROR_TEMPLATE, "Vault is in performance standby", response.getPayload());
                    monitor.debug(performanceStandbyMsg);
                    return HealthCheckResult.failed(performanceStandbyMsg);
                case NOT_INITIALIZED:
                    final String notInitializedMsg = String.format(HEALTH_CHECK_ERROR_TEMPLATE, "Vault is not initialized", response.getPayload());
                    monitor.debug(notInitializedMsg);
                    return HealthCheckResult.failed(notInitializedMsg);
                case SEALED:
                    final String sealedMsg = String.format(HEALTH_CHECK_ERROR_TEMPLATE, "Vault is sealed", response.getPayload());
                    monitor.debug(sealedMsg);
                    return HealthCheckResult.failed(sealedMsg);
                case UNSPECIFIED:
                default:
                    final String unspecifiedMsg = String.format(HEALTH_CHECK_ERROR_TEMPLATE, "Unspecified response from vault. Code: " + response.getCode(), response.getPayload());
                    monitor.debug(unspecifiedMsg);
                    return HealthCheckResult.failed(unspecifiedMsg);
            }

        } catch (IOException e) {
            final String exceptionMsg = String.format(HEALTH_CHECK_ERROR_TEMPLATE, "IOException: " + e.getMessage(), "");
